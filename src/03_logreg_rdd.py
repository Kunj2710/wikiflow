"""
03_logreg_rdd.py
Logistic regression from scratch using PySpark RDDs and mini-batch SGD.
No MLlib, no spark.ml.

Args:
    --features  gs://.../features/
    --out       gs://.../models/logreg/  (writes weights.json + scaler.json)
    --metrics   gs://.../metrics/logreg.json
    --epochs    default 30
    --lr        default 0.1
    --batch     default 4096
"""
...
def standardize_stats(rdd_features):
    """Compute mean and std of every feature across the dataset."""
    n = rdd_features.count()
    sums = rdd_features.reduce(lambda a, b: [x + y for x, y in zip(a, b)])
    means = [s / n for s in sums]
    sq = rdd_features.map(
        lambda v: [(x - m) ** 2 for x, m in zip(v, means)])
...
def gradient_for_batch(batch, w, b):
    """Compute sum of gradients on a batch."""
    g_w = [0.0] * len(w)
    g_b = 0.0
...
def main():
    ...
    spark = (SparkSession.builder
             .appName("WikiFlow-03-logreg-rdd")
             .getOrCreate())
    sc = spark.sparkContext

    df = spark.read.parquet(args.features)

    # 80/20 split, deterministic by seed.
    train_df, test_df = df.randomSplit([0.8, 0.2], seed=args.seed)
    train_df.cache()
    test_df.cache()

    # Convert to RDD of (label, [features]).
    def row_to_pair(r):
        return (float(r["dropout"]),
    ...
    feat_only = train.map(lambda x: x[1])
    feat_only.cache()
    ...
    for epoch in range(args.epochs):
        # Sample a fraction equivalent to one batch on average per partition.
        # treeAggregate sums gradients distributed.
        ...
        def combine(a, b_):
            gw_a, gb_a, n_a = a
            gw_b, gb_b, n_b = b_
            return ([x + y for x, y in zip(gw_a, gw_b)],
                    gb_a + gb_b,
                    n_a + n_b)
    ...
    # AUC by Mann-Whitney U on sampled pos/neg pairs to keep it cheap.
    ...
    sc.parallelize([json.dumps(weights, indent=2)], 1) \
      .saveAsTextFile(args.out.rstrip("/") + "/weights.tmp")
    sc.parallelize([json.dumps(scaler, indent=2)], 1) \
      .saveAsTextFile(args.out.rstrip("/") + "/scaler.tmp")
    # Spark writes a folder; the eval script knows to read part-00000.
