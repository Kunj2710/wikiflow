"""
04_kmeans_rdd.py
K-Means from scratch on PySpark RDDs. Broadcast centroids, assign, recompute,
repeat. No MLlib.

Args:
    --features  gs://.../features/
    --out       gs://.../models/kmeans/
    --metrics   gs://.../metrics/kmeans.json
    --k         number of clusters (default 4)
    --iters     max iterations (default 30)
"""
...
def main():
    ...
    spark = (SparkSession.builder
             .appName("WikiFlow-04-kmeans-rdd")
             .getOrCreate())
    ...
    pts = pts.map(
        lambda v: [(x - m) / s
                   for x, m, s in zip(v, bc_means.value, bc_stds.value)]
    ).cache()

    # Init: k random points sampled from the data.
    init = pts.takeSample(False, args.k, seed=args.seed)
    centers = [list(p) for p in init]
    ...
    for it in range(args.iters):
        ...
        def merge(a, b):
            sum_a, n_a, cost_a = a
            sum_b, n_b, cost_b = b
            return ([x + y for x, y in zip(sum_a, sum_b)],
                    n_a + n_b,
                    cost_a + cost_b)
    ...
    # Silhouette on the sample (full silhouette is O(n^2)).
    sil_scores = []
    for p in sample:
        dists = [sum((p[i] - c[i]) ** 2 for i in range(len(p)))
                 for c in centers]
    ...
    # Mean feature vector per cluster (in original units, undo standardization).
    ...
    def merge2(a, b):
        sa, na = a
        sb, nb = b
        return ([x + y for x, y in zip(sa, sb)], na + nb)

    summaries = (pts.map(assign_cluster)
                 .reduceByKey(merge2)
                 .collect())
    ...
    for cluster_id, (s, n) in summaries:
        ...
        original_mean = [m + sc_ * mi
                         for m, sc_, mi in zip(bc_means.value, scaled_mean,
                                               bc_stds.value)]
        # The line above mixes order; redo cleanly:
    ...
    sc.parallelize([json.dumps(centroids_payload, indent=2)], 1) \
      .saveAsTextFile(args.out.rstrip("/") + "/centers.tmp")
