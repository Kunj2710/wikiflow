...
Pull all per-model metric files from GCS, build a single Plotly HTML
dashboard, write it back to GCS, and export a flat results table to BigQuery
so anyone can query the project results without Spark.
...
def read_json_folder(spark, path):
    """metrics paths are written as Spark text folders; concatenate parts."""
...
def main():
    ...
    spark = (SparkSession.builder
             .appName("WikiFlow-08-eval-dashboard")
             .getOrCreate())
    ...
    try:
        dnn = read_json_folder(spark, args.metrics + "dnn.json.tmp")
    except Exception as e:
        print("DNN metrics not found, skipping:", e)
        dnn = None

    # ---- Cohort summary from features -----------------------------------
    ...
    aucs = [logreg["auc_roc"]]
    if dnn is not None:
        names.append("Elephas DNN")
        aucs.append(dnn["auc_roc"])
    ...
    fig.add_trace(go.Scatter(x=rv, y=vv, mode="markers+text",
                             ...
                             marker=dict(size=18, color=sizes,
                                         colorscale="Viridis"),
                             name="clusters"),
                  row=2, col=2)
    ...
    # Write the HTML to GCS via Hadoop FileSystem.
    ...
    p_obj = sc._jvm.org.apache.hadoop.fs.Path(out_path)
    out_stream = fs.create(p_obj, True)
    out_stream.write(html.encode("utf-8"))
    out_stream.close()
    print(f"Wrote {out_path}")

    # Also drop the summary JSON next to it.
    j_obj = sc._jvm.org.apache.hadoop.fs.Path(
        args.dashboard.rstrip("/") + "/summary.json")
    j_stream = fs.create(j_obj, True)
    j_stream.write(json.dumps(summary, indent=2).encode("utf-8"))
    j_stream.close()

    # ---- Export results table to BigQuery -------------------------------
    ...
    project = sc.getConf().get("spark.app.name").split("-")[0] or "wikiflow"

    # The Spark BigQuery connector picks up the project from the cluster.
    (results_df.write
        .format("bigquery")
    ...
    print("Pushed results to BigQuery:",
